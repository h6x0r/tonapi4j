...
public class AsyncTonapiClientBase {
  ...
  public AsyncTonapiClientBase(String apiKey,
    ...
    this.websocketUrl = Optional.ofNullable(websocketUrl)
        .orElse(this.isTestnet ? "wss://testnet.tonapi.io/v2/websocket"
   ...
   * @throws TONAPIError If the status code is not 200, deserialization fails or received response
   *                     is not valid.
   */
        ...
        try {
          requestAsync(method, path, headers, params, body, responseType)
              ...
              .exceptionally(e -> {
                ...
                if (cause instanceof TONAPITooManyRequestsError
                    && attempt.incrementAndGet() <= maxRetries) {
                  log.warn("Rate limit exceeded. Retrying {}/{} in 1 second.", attempt.get(),
                      maxRetries);
                  try {
                    Thread.sleep(1000);
                    ...
                    future.completeExceptionally(
                        new TONAPIError("Thread interrupted during retry sleep.",
