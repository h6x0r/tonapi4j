...
public class InscriptionsMethod extends TonapiClientBase {
  ...
  /**
   * Get all inscriptions by owner address. It's an experimental API and can be dropped in the
   * future.
   *
  ...
  /**
   * Get the transfer inscriptions history for an account. It's an experimental API and can be
  ...
  /**
   * Get the transfer inscriptions history for an account filtered by ticker. It's an experimental
   * API and can be dropped in the future.
   *
   * @param accountId      Wallet address
  ...
  public AccountEvents getInscriptionHistoryByTicker(
      ...
      int limit,
      String acceptLanguage) throws TONAPIError {
    String method = String.format("v2/experimental/accounts/%s/inscriptions/%s/history", accountId,
        ticker);
    Map<String, Object> params = new HashMap<>();
    params.put("limit", limit);
    if (beforeLt != null) {
  ...
  /**
   * Return comment for making operation with inscription. Please don't use it if you don't know
   * what you are doing.
   *
